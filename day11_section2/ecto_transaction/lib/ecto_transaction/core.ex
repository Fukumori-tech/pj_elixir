defmodule EctoTransaction.Core do
  ...
  def create_user(params) do
    ...
    |> Multi.insert(:point, fn %{user: user} -> Point.changeset(%Point{}, %{user_id: user.id, point_balance: @initial_points}) end)
    |> Multi.insert(:point_log, fn %{user: user} -> PointLog.changeset(%PointLog{}, %{user_id: user.id, category: "give", amount: @initial_points, point_balance: @initial_points})end)
    |> Repo.transaction()
  end
  ...
  def buy_item(user_id, item_id) do
    ...
    if user && item do
      Multi.new()
      |> Multi.insert(:get_item, %GetItem{user_id: user.id, item_id: item.id}) #userに束縛されているのがすでにidではない？
      |> Multi.update(:point, fn _ ->
        #pointsテーブルのpoint_balanceからアイテムのpriceを引いた値を計算
  ...
  def delete_user(user_id) do
    ...
    |> Multi.run(:user, fn repo, _ ->
      ...
      |> Multi.delete(:delete_user, fn %{ user: user} ->
        user
      end)
